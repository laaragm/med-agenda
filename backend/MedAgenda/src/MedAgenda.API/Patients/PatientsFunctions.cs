...
namespace MedAgenda.API.Patients
{
    public class Patients : FunctionBase
	{
        ...
        public async Task<HttpResponseData> CreatePatient([HttpTrigger(AuthorizationLevel.Function, "POST", Route = Route)] HttpRequestData req, CancellationToken cancellationToken)
        {
            try
            {
                _logger.LogInformation("Triggered CreatePatient function.");
				var patient = await JsonSerializer.DeserializeAsync<CreatePatientRequest>(req.Body);
				if (patient is null)
			    {
